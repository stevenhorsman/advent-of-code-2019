[string]$template_day="01"
[string]$template_name="rocket_equation"
[string]$day=$args[0]
$day=$day.PadLeft(2,'0')
[string]$name=$args[1]
...
(Get-Content .\day-$day\$name.py).replace($template_name, $name).replace($template_day, $day) | Set-Content .\day-$day\$name.py
(Get-Content .\day-$day\test_$name.py).replace($template_name, $name).replace($template_day, $day) | Set-Content .\day-$day\test_$name.py